...
def test_refresh_search_text_reuses_connection_for_single_artifact(tmp_path):
    ...
    store._conn().execute(
        """
        UPDATE raw_index_artifact_strings
        SET value = ?
        WHERE artifact_id = ? AND field_name = ?
        """,
        ("bravo.exe", artifact_id, "Name"),
    )
    store._conn().commit()
    original_conn = store._conn
    conn_calls = 0
    ...
    refreshed = store._refresh_search_text(artifact_id)
    search_text = original_conn().execute(
        """
        SELECT search_text
        FROM raw_index_search_text
        WHERE artifact_id = ?
        """,
        (artifact_id,),
    ).fetchone()[0]
    ...
    assert "bravo.exe" in search_text
    assert conn_calls == 1


def test_write_search_text_uses_explicit_connection_for_fts_lookup(tmp_path):
    db_path = tmp_path / "raw-index.sqlite"
    store = RawIndexStore(str(db_path))
    store.open()
    ...
    updated = store._write_search_text(
        artifact_id,
        "manual refresh text",
        conn=explicit_conn,
    )
...
def test_search_offset_past_total_skips_page_hydration_queries(tmp_path):
    db_path = tmp_path / "raw-index.sqlite"
    store = RawIndexStore(str(db_path))
    store.open()
...
def test_search_text_freshness_check_reuses_connection_on_cache_miss(tmp_path):
    db_path = tmp_path / "raw-index.sqlite"
    store = RawIndexStore(str(db_path))
    store.open()
    ...
    rebuilt = store._ensure_search_text_current()
...
def test_fast_candidate_probe_cache_ignores_or_keyword_order(tmp_path):
    db_path = tmp_path / "raw-index.sqlite"
    store = RawIndexStore(str(db_path))
    store.open()
    ...
    assert first["search_strategy"]["index"] == "fts5_trigram_or"
    assert second["search_strategy"]["index"] == "fts5_trigram_or"
    assert len(fast_candidate_probes) == 1
...
def test_date_filtered_search_reuses_fast_candidate_probe_for_untimed_check(tmp_path):
    db_path = tmp_path / "raw-index.sqlite"
    store = RawIndexStore(str(db_path))
    store.open()
    ...
    assert result["total"] == 905
    assert result["search_strategy"]["fast_candidate_gap"] == (
        "fast_candidate_too_large"
    )
    assert len(fast_candidate_probes) == 1
...
def test_date_filtered_search_page_query_avoids_temp_order_sort(tmp_path):
    db_path = tmp_path / "raw-index.sqlite"
    store = RawIndexStore(str(db_path))
    store.open()
