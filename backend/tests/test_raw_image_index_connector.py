...
def test_raw_image_index_connector_timeline_count_uses_time_range_index(tmp_path):
    db_path = tmp_path / "raw-index.sqlite"
    _seed_timed_types(
        db_path,
        ["File System Entry", "Registry Entry", "File System Entry"],
...
def test_raw_image_index_connector_hot_keyword_untimed_probe_uses_fts_join(tmp_path):
    db_path = tmp_path / "raw-index.sqlite"
    store = RawIndexStore(str(db_path))
    store.open()
    fts_exists = store._conn().execute(
