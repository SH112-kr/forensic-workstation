...
- E01, VM, raw 디스크 이미지를 마운트하고 정적 파일 추출 및 raw artifact 쿼리를 수행합니다.
...
| `dissect.target` | E01/VM/raw 이미지 마운트 | 이미지 브라우징, raw image 파일 추출 차단 |
...
| `pyshark` + `tshark` | 선택적 PCAP 분석 | PCAP 전용 네트워크 대화, DNS, HTTP, IOC 추출 차단. 누락되어도 endpoint E01 기본 분석 준비 상태를 degraded로 낮추지 않습니다. |
...
| 디스크 이미지 | `.E01`, `.VMDK`, `.VHDX`, `.VDI`, `.QCOW2`, raw image | 정적 추출 및 raw artifact 도구 지원, 전체 탐지는 KAPE 파싱 권장 |
...
| 문서 파일 | `.docx`, `.hwp`, `.pdf`, `.txt` | 경로/메타데이터/타임스탬프 컨텍스트만 사용, 본문 추출과 열람 차단 |
| 바이너리 | `.exe`, `.dll` | 정적 Ghidra 분석 전용, 추출 파일 실행 금지 |
| 로그 | IIS, Apache, syslog 계열 | import 후 키워드/status/IP 검색 |
| 네트워크 캡처 | `.pcap`, `.pcapng` | 선택적 보조 증거. 사용할 때만 `pyshark`, `tshark` 필요 |
...
디스크 이미지만 로드된 경우에는 파싱된 아티팩트가 있다고 가정하지 말고 먼저 소스 커버리지를 확인하세요.
